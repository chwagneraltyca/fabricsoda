 ...
 *
 * ACTIVE USAGE IN THIS PROJECT:
 * - POWERBI_API: Used by LineageService for GraphQL API access
 * - Other scopes: Available for future use (e.g., Fabric REST API, OneLake)
 *
 * NOTE: Most scope combinations (SCOPES.*) are prepared for future use.
 * Currently, only FABRIC_BASE_SCOPES.POWERBI_API is actively used.
 */
...
export const FABRIC_BASE_SCOPES = {
  ...
  // Power BI / Analysis Services API (used for GraphQL API)
  // The GraphQL API uses Power BI API scope, not Fabric API scope
  // ✅ ACTIVELY USED by LineageService for GraphQL connectivity
  POWERBI_API: "https://analysis.windows.net/powerbi/api/.default",
  GRAPHQL_EXECUTE: "https://analysis.windows.net/powerbi/api/GraphQLApi.Execute.All",
};
