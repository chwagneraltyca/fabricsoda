-- ============================================================================
-- DQ CHECKER - Simplified Schema (Suites + N:M + Checks)
-- ============================================================================
-- Based on: SIMPLIFICATION-ANALYSIS.md v3
-- Changes:
--   - Added dq_suites (business metadata)
--   - Added suites_testcases (N:M link)
--   - Updated dq_testcases (added schema_name, table_name from Contract)
--   - Simplified dq_checks (removed redundant source/schema/table)
--   - Added vw_orphan_testcases view
--   - Added suite management SPs
-- ============================================================================
-- ELIMINATED: dq_contracts (merged into dq_testcases)
-- ============================================================================
...
DROP PROCEDURE IF EXISTS dbo.sp_create_suite;
DROP PROCEDURE IF EXISTS dbo.sp_update_suite;
DROP PROCEDURE IF EXISTS dbo.sp_delete_suite;
DROP PROCEDURE IF EXISTS dbo.sp_add_testcase_to_suite;
DROP PROCEDURE IF EXISTS dbo.sp_remove_testcase_from_suite;
DROP PROCEDURE IF EXISTS dbo.sp_create_testcase;
DROP PROCEDURE IF EXISTS dbo.sp_update_testcase;
DROP PROCEDURE IF EXISTS dbo.sp_delete_testcase;
GO
...
-- Note: source_id, schema_name, table_name now come from testcase
...
-- Orphan testcases (not in any suite) - for management
...
-- Suite testcases with full details
...
SELECT
    ...
    sch.column_types,
    sch.column_indexes,
    sch.warn_required_missing,
    sch.warn_forbidden_present,
    sch.warn_wrong_type,
    sch.warn_wrong_index,
    sch.fail_required_missing,
    sch.fail_forbidden_present,
    sch.fail_wrong_type,
    sch.fail_wrong_index,
    -- Reference extension
    r.reference_table,
    r.reference_column,
...
-- ============================================================================
-- STORED PROCEDURES: Suite CRUD
-- ============================================================================

CREATE OR ALTER PROCEDURE dbo.sp_create_suite
    @suite_name NVARCHAR(255),
    @suite_code NVARCHAR(50) = NULL,
...
BEGIN
    ...
    -- Auto-generate suite_code if not provided
    IF @suite_code IS NULL
    BEGIN
        DECLARE @next_id INT;
        SELECT @next_id = ISNULL(MAX(suite_id), 0) + 1 FROM dbo.dq_suites;
        SET @suite_code = 'SUITE_' + RIGHT('00000' + CAST(@next_id AS VARCHAR(5)), 5);
    END

    INSERT INTO dbo.dq_suites (
        suite_name, suite_code, description, category, data_domain,
        execution_order, owner, tags, is_active
    )
    VALUES (
        @suite_name, @suite_code, @description, @category, @data_domain,
        @execution_order, @owner, @tags, @is_active
    ...
    SELECT * FROM dbo.dq_suites WHERE suite_id = SCOPE_IDENTITY();
END;
GO

CREATE OR ALTER PROCEDURE dbo.sp_update_suite
    @suite_id INT,
    @suite_name NVARCHAR(255),
    @suite_code NVARCHAR(50) = NULL,
    @description NVARCHAR(1000) = NULL,
...
BEGIN
    ...
    UPDATE dbo.dq_suites
    SET suite_name = @suite_name,
        suite_code = ISNULL(@suite_code, suite_code),
        description = @description,
        category = @category,
    ...
    SELECT * FROM dbo.dq_suites WHERE suite_id = @suite_id;
END;
GO

CREATE OR ALTER PROCEDURE dbo.sp_delete_suite
    @suite_id INT
AS
BEGIN
    SET NOCOUNT ON;

    -- suites_testcases rows cascade delete
    DELETE FROM dbo.dq_suites WHERE suite_id = @suite_id;
...
-- ============================================================================
-- STORED PROCEDURES: Suite-Testcase Link Management
-- ============================================================================

CREATE OR ALTER PROCEDURE dbo.sp_add_testcase_to_suite
    @suite_id INT,
    @testcase_id INT
AS
BEGIN
    ...
    IF NOT EXISTS (
        SELECT 1 FROM dbo.suites_testcases
        WHERE suite_id = @suite_id AND testcase_id = @testcase_id
    )
    BEGIN
    ...
    SELECT @suite_id AS suite_id, @testcase_id AS testcase_id, 'added' AS result;
END;
GO

CREATE OR ALTER PROCEDURE dbo.sp_remove_testcase_from_suite
    @suite_id INT,
    @testcase_id INT
AS
BEGIN
    ...
    DELETE FROM dbo.suites_testcases
    WHERE suite_id = @suite_id AND testcase_id = @testcase_id;

    SELECT @@ROWCOUNT AS removed_count;
END;
GO
...
CREATE OR ALTER PROCEDURE dbo.sp_create_testcase
    ...
    @owner NVARCHAR(100) = NULL,
    @tags NVARCHAR(500) = NULL,
    @suite_id INT = NULL  -- Optional: link to suite immediately
AS
BEGIN
    SET NOCOUNT ON;
    ...
    -- If suite_id provided, link immediately
...
CREATE OR ALTER PROCEDURE dbo.sp_update_testcase
    @testcase_id INT,
    @testcase_name NVARCHAR(255),
    @source_id INT,
    @schema_name NVARCHAR(128),
...
CREATE OR ALTER PROCEDURE dbo.sp_delete_testcase
    @testcase_id INT
AS
BEGIN
    SET NOCOUNT ON;

    -- suites_testcases and dq_checks cascade delete
    DELETE FROM dbo.dq_testcases WHERE testcase_id = @testcase_id;
...
PRINT 'DQ Checker Simplified schema deployed successfully!';
PRINT 'Tables: dq_sources, dq_suites, suites_testcases, dq_testcases, dq_checks + 5 extension tables';
PRINT 'Views: vw_active_data_sources, vw_orphan_testcases, vw_suite_testcases, vw_checks_complete, vw_fabric_*';
PRINT 'Stored Procedures: 20 SPs for CRUD operations';
GO
