...
$WorkloadDir = Join-Path $ProjectRoot "src\Workload"
$ManifestDir = Join-Path $ProjectRoot "build/Manifest"


# Check if .env file exists (should be created from .env.template)
$EnvDevFile = Join-Path $WorkloadDir ".env"
if (-not (Test-Path $EnvDevFile)) {
    Write-Error "Environment file not found: $EnvDevFile"
    Write-Error "Please copy .env.template to .env and configure it."
    exit 1
}

# Load configuration from .env file
Write-Host "📖 Loading project configuration from .env..." -ForegroundColor Blue
$EnvConfig = @{}
