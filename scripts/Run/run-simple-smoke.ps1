# Run smoke_test_simple notebook in Fabric
...
Write-Host "=== Running Simple Smoke Test ===" -ForegroundColor Cyan
...
$notebook = $items.value | Where-Object { $_.displayName -eq "smoke_test_simple" }

if (-not $notebook) {
    Write-Host "ERROR: smoke_test_simple notebook not found!" -ForegroundColor Red
    exit 1
}
